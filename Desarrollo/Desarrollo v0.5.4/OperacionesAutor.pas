...
begin
  ...
  begin
    ...
    edit2.Text:=datamodule1.adoautor.FieldByName('Nombre').AsString;
    edit3.Text:=datamodule1.adoautor.FieldByName('Apellido').AsString;
    edit1.Text:=datamodule1.adoautor.FieldByName('DNI').Asinteger;
    edit4.Text:='';
  end;
end;
...
begin
  ...
  if buttonSelected= mrOk then Begin
    try
      ...
      if (not unit1.DataModule1.comprobarautor.isempty) then
        raise Edniduplicado.Create('Ya existe un autor con ese DNI');
      datamodule1.adoautor.FieldByName('DNI').Asinteger:= strtoint(edit3.Text);
...
begin
  if (not unit1.DataModule1.ADOAutor.IsEmpty) then
