...
type
  ...
  EContraseniasNoCoinciden=class(Exception);
...
procedure TFormRegistroCliente.Edit15Change(Sender: TObject);
begin
...
begin
  ...
  if buttonSelected= mrOk then
    begin
      ...
      DataModule1.ComprobarUsuario.Parameters.ParamByName('user').Value:=Edit4.Text;
      ...
      if (Edit4.Text = '') then
        ...
        if not ((Datamodule1.ComprobarUsuario.ISEmpty)) then         //Existe usuario, es otro usuario.
           raise EExisteUsuario.Create('El usuario ya se encuentra en la base de datos')
        else
            if not (Datamodule1.ComprobarUsuarioAdmin.ISEmpty) then                //Existe usuario, ES ADMIN!!!
                raise EExisteUsuario.Create('El usuario ya se encuentra en la base de datos')
            else
      ...
      if (Edit5.Text = '') then
         raise EEspacioBlanco.Create('Complete los campos en rojo')
      	...
      	if (Edit15.Text = '') then
      		...
      		if (Edit5.Text <> Edit15.Text) then //Comparo Claves
      			raise EContraseniasNoCoinciden.Create('Las contraseñas no coinciden')
      			else
              //Si son igual
      ...
      if (Edit11.Text = '') then
        ...
        if EncontrarArroba(Edit11.Text)=False then
          raise ESinArroba.Create('El email no es válido')
        else
     ...
     { if (Edit12.Text = '') then
        ...
        if (Edit13.Text = '') then                                          TELEFONO AHORA NO ES OBLIGATORIO
           raise EEspacioBlanco.Create('Complete los campos en rojo')       QUE GANAS DE JODER QUE TENES NEG
        else
          if (Edit13.Text = '') then
             raise EEspacioBlanco.Create('Complete los campos en rojo')
          else begin
            ...
            DataModule1.ADOCliente.FieldByName('Telefono').AsString:=Telefono;
            //end;
      if (DateTimePicker1.Date<1900)or (DateTimePicker1.Date>Now()) then
        raise EFechaNac.Create('Ingrese correctamente su fecha de nacimiento');
           DataModule1.ADOCliente.FieldByName('Fecha Nacimiento').AsDateTime:=DateTimePicker1.Date;
      DataModule1.ADOCliente.FieldByName('FechaDeInicioCliente').AsDateTime:=Now();
      DataModule1.ADOCliente.Post;
      ShowMessage('El usuario se dio de alta exitosamente');
      //Restablezco valores luego de agregarlo!
  		FormRegistroCliente.RestablecerValores;
      DateTimePicker1.Date:=Now(); //Vuelvo al dia de hoy
      FormRegistroCliente.Close;
     ...
     on E:EEspacioBlanco do begin
      ShowMessage( E.Message);
        ...
        if (Edit15.Text = '') then begin Label16.Font.Color:= clRed;label15.Visible:=True; end;//Confirmar Clave
        ...
        if DateTimePicker1.Date = Now() then begin Label3.Font.Color:=clRed; label15.Visible:=True; end;//Fecha Nac
     ...
     on E:EContraseniasNoCoinciden do begin
          ShowMessage(E.Message);
